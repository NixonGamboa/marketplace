...
No aplica en el Sprint DEMO (sin backend). En Sprint 1+, cada pedido tendrá un `orderId` (ULID/NanoID) que actúa como trace ID a través de todos los sistemas (PWA → API → BD → WhatsApp). Ver `adr-001-stack-backend.md` §4 sobre el uso de ULID para preservar portabilidad futura.
